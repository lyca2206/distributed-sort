...
public class MSDRadixSortTask extends RecursiveAction {
    ...
    private void sort() { // Sort from a[lo] to a[hi], starting at the dth character.
        ...
        for (int i = lo; i <= hi; i++) 
            aux[count[charAt(array[i], d) + 1]++ + lo] = array[i];//+ lo  para que se pueda hacer a[i] = aux[i]
        
        // Copy back --> Copies aux array to original array    
        System.arraycopy(aux, lo, array, lo, hi + 1 - lo); // Not slow enough to paralelize
